...
class Login
{
	...
	public static function check_access_token($token) {
		...
		if (hash_equals(self::secrets()['access'], (string) $decrypted['secret']) && $decrypted['expires'] > time()) {
			return true;
		}
		return false;
	}
	...
	public static function check_refresh_token($token) {
		...
		if ($decrypted !== false && hash_equals(self::secrets()['refresh'], (string) $decrypted['secret']) && $decrypted['expires'] > time()) {
			return true;
		}
		return false;
	}
